...
/* 90度単位に丸めて 0/90/180/270 にする。ページ全体の回転など、直角しか扱わない所で使う */
const norm360 = d => ((Math.round(d / 90) * 90 % 360) + 360) % 360;
/* 丸めずに 0〜359 へ収める。写真の自由角度で使う */
const normAngle = d => (((+d || 0) % 360) + 360) % 360;
...
/** rot は数値（時計回りの度数・任意）か、テンプレート側の 'cw'/'ccw'/'auto-cw'/'auto-ccw' */
function applyRotate(src, rot) {
  ...
  /* 直角以外に回すと四隅が透明になる。そのまま枠に敷くと角が白く抜けるので、
     透明を含まない最大の範囲だけを残す。外接矩形と同じ形で切ると、
     直角のときは元のまま（切り落としゼロ）になる。 */
  const A = (w * c + h * sn) / (w * sn + h * c);     // 回した後の外接矩形の縦横比
  const W = Math.min(w / (c + sn / A), h / (sn + c / A));
  const cw = Math.max(1, Math.round(W)), ch = Math.max(1, Math.round(W / A));
...
function slotRotation(pg, slot) {
  ...
  return r == null ? pageRotation(pg) : normAngle(r);
}

function setSlotRot(pg, slot, deg) {
  pg.rots = pg.rots || [];
  pg.rots[slot] = normAngle(deg);        // 0も明示的に持つ（ページ指定を打ち消すため）
}
...
/** ↺↻ の90度ずつ。1枚だけの版面なら、対になる縦向きの版面へ入れ替える */
function rotateSlot(slot, step) {
  const pg = S.pages[S.sel]; if (!pg) return;
  const t = T[pg.template] || {};
  const cur = slotRotation(pg, slot);
  const next = norm360(cur + step);
  if ((t.slots || 0) === 1) {
    const wasQ = cur === 90 || cur === 270, isQ = next === 90 || next === 270;
    if (wasQ !== isQ) swapTemplateForRotation(pg, isQ);
  }
  setSlotRot(pg, slot, next);
  renderIns(); touch(false);
}

/** スライダーからの自由角度。作り直すと掴んでいるつまみが外れるので、
    表示だけ書き換えて絵を描き直す */
function setSlotAngle(slot, deg) {
  const pg = S.pages[S.sel]; if (!pg) return;
  setSlotRot(pg, slot, deg);
  const l = $(`#ra${slot}`); if (l) l.textContent = `${normAngle(deg)}°`;
  const cv = $('#insPrev'); if (cv) drawPage(cv, pg, printNo(S.sel), 2.6);
  clearTimeout(setSlotAngle._t);
  setSlotAngle._t = setTimeout(() => { renderGrid(); drawPreview(); }, 120);
}
...
/** 長さ len の並びを、実際にめくったときの見え方で組にする。
    表紙(1ページ目)と裏表紙(最終ページ)は相手がいないので単独。
    間は 2|3, 4|5 … と組になる。
    **渡す長さは4の倍数であること**（pagesForPrint() / printSeq() は必ずそうなっている）。
    半端な数を渡すと最後の組が裏表紙を巻き込み、単独の裏表紙と二重に出る。 */
function spreadsOf(len) {
  if (len <= 0) return [];
  if (len === 1) return [[0]];
  const out = [[0]];
  for (let i = 1; i < len - 1; i += 2) out.push([i, i + 1]);
  if (len % 2 === 0) out.push([len - 1]);   // 奇数なら最後の組に吸収済み
  return out;
}

function bookSpreads(list) { return spreadsOf((list || pagesForPrint()).length); }

/** 台割を見開きの組にする。要素は { i, no } で、
    i は S.pages の添字（-1 は書き出し時に補う白ページ）、no は刷ったときのページ番号。 */
function gridSpreads() {
  const seq = printSeq();
  return spreadsOf(seq.length).map(sp => sp.map(k => ({ i: seq[k], no: k + 1 })));
}
...
function redrawPages() {
  renderGrid();
  const p = $('#insPrev');
  if (p && S.sel >= 0) drawPage(p, S.pages[S.sel], printNo(S.sel), 2.6);
  drawPreview();
}
...
function setColor(field, v, src) {
  ...
  setColor._t = setTimeout(() => {
    renderGrid();
    const p = $('#insPrev'); if (p && S.sel >= 0) drawPage(p, S.pages[S.sel], printNo(S.sel), 2.6);
    drawPreview();
  }, 150);
}
...
function printSeq() {
  const n = S.pages.length, need = blanksNeeded(n);
  const seq = S.pages.map((_, i) => i);
  if (!need) return seq;
  const at = Math.max(1, n - 1);         // 裏表紙の手前。1ページしか無いなら表紙の後ろ
  for (let i = 0; i < need; i++) seq.splice(at, 0, -1);   // -1 = 補う白ページ
  return seq;
}

function pagesForPrint() {
  if (!blanksNeeded(S.pages.length)) return S.pages;
  return printSeq().map(i => (i < 0 ? { template: 'blank' } : S.pages[i]));
}

/** 台割の添字 i が、刷ったときに何ページ目になるか。
    白ページは裏表紙の手前に入るので、ずれるのは裏表紙だけ。 */
function printNo(i) {
  const n = S.pages.length, need = blanksNeeded(n);
  return (need && n > 1 && i === n - 1) ? n + need : i + 1;
}
...
/** カードに出す角度。写真を置く版面なら1枚目の角度、そうでなければページ全体の角度 */
function cardAngle(pg) {
  const t = T[pg.template];
  return (t && (t.slots || 0) > 0) ? slotRotation(pg, 0) : pageRotation(pg);
}

/** 見開きひと組ぶんのHTML。とじ方向に従って、近い側（左／上）を先に置く。
    相手のいない表紙・裏表紙は、反対側を空けて左右の関係が見えるようにする。 */
function spreadHtml(cells) {
  ...
  return `<div class="sp">${side.map(c => {
    ...
    if (c.i < 0) return `
      <div class="card ghost" title="4の倍数にするため、書き出し時に入る白ページです">
        ...
        <div class="meta"><span class="no">${c.no}</span><span class="tn">白ページ（自動）</span></div>
      </div>`;
    const pg = S.pages[c.i];
    return `
      <div class="card ${c.i === S.sel && !S.pickPages ? 'sel' : ''} ${S.markedPages.has(c.i) ? 'mark' : ''}"
           draggable="${!S.pickPages}" data-i="${c.i}" data-no="${c.no}">
        <canvas class="ph"></canvas>
        <span class="side">${ORIENT_NAME === 'landscape'
          ? (isFarPage(c.no) ? '下' : '上') : (isFarPage(c.no) ? '右' : '左')}</span>
        ${cardAngle(pg) ? `<span class="rot">${cardAngle(pg)}°</span>` : ''}
        ${S.pickPages ? '<span class="chk"></span>' : ''}
        <div class="meta"><span class="no">${c.no}</span><span class="tn">${esc(pg.template)}</span></div>
      </div>`;
  }).join('')}</div>`;
}
...
function renderGrid() {
  ...
  for (const c of g.querySelectorAll('.card.ghost')) {
    drawPage(c.querySelector('canvas'), { template: 'blank' }, +c.querySelector('.no').textContent, 1.35);
    c.ondragover = e => {
      if (DRAG.kind !== 'page') return;
      e.preventDefault(); e.dataTransfer.dropEffect = 'move';
      clearDropMarks(); c.classList.add('dropOn');
      DRAG.to = S.pages.length - 1;          // 裏表紙の手前へ
    };
    c.ondrop = e => {
      if (DRAG.kind !== 'page') return;
      e.preventDefault(); e.stopPropagation();
      const { from, to } = DRAG; endDrag();
      if (movePageTo(from, to)) renderAll();
    };
  }

  for (const c of g.querySelectorAll('.card:not(.ghost)')) {
    const i = +c.dataset.i;
    drawPage(c.querySelector('canvas'), S.pages[i], +c.dataset.no, 1.35);
    c.onclick = e => {
    ...
    c.ondragover = e => {
      if (DRAG.kind === 'page') {
        ...
        /* カードの左半分なら手前、右半分なら後ろに入る（横ページは上下で見る） */
        const r = c.getBoundingClientRect();
        const after = ORIENT_NAME === 'landscape'
